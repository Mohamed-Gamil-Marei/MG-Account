...
import { WhatsAppDocumentShareModal, DocShareType } from './archive/WhatsAppDocumentShareModal';
...
export const WhatsAppBotView: React.FC<WhatsAppBotViewProps> = ({
  ...
  // File & Document Sharing Modal State
  const [isShareModalOpen, setIsShareModalOpen] = useState(false);
  const [shareModalType, setShareModalType] = useState<DocShareType>('ARCHIVED_DOCUMENT');
  const [isCodeModalOpen, setIsCodeModalOpen] = useState(false);
  const [showAttachMenu, setShowAttachMenu] = useState(false);

  const handleOpenShareModal = (type: DocShareType) => {
  ...
  return (
    <div className="space-y-4">
        ...
        >
          ...
          {selectedClient ? (
            <div className="p-3.5 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between gap-2 z-10">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  id="btn-header-send-code"
                  onClick={() => setIsCodeModalOpen(true)}
                  className="px-2.5 py-1.5 rounded-xl text-xs font-bold bg-teal-800 hover:bg-teal-700 text-teal-100 flex items-center gap-1.5 transition-all shadow-xs cursor-pointer border border-teal-700"
                  title="إرسال كود تحقق أو اعتماد عبر WhatsApp من داخل البرنامج"
                >
                  <KeyRound className="w-3.5 h-3.5 text-teal-300" />
                  <span className="hidden sm:inline">إرسال كود 🔐</span>
                </button>
          ...
          <div className="p-3 bg-slate-900 border-t border-slate-800 flex items-center gap-2">
            ...
            <div className="relative">
              ...
              {showAttachMenu && (
                <div className="absolute bottom-full right-0 mb-2 w-64 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-2 z-50 text-right animate-in zoom-in-95 space-y-1">
                  <div className="text-[10px] font-bold text-slate-400 px-2 py-1 border-b border-slate-800 mb-1 flex items-center justify-between">
                    <span>إرسال ملفات ومستندات عبر WhatsApp:</span>
                    <button
                      type="button"
                  ...
                  <button
                    type="button"
                    onClick={() => handleOpenShareModal('ARCHIVED_DOCUMENT')}
                    className="w-full p-2 rounded-xl hover:bg-purple-950/50 hover:text-purple-300 text-slate-200 text-xs flex items-center gap-2.5 transition-colors cursor-pointer text-right"
                  >
                    <FileText className="w-4 h-4 text-purple-400 shrink-0" />
                    <div>
                      <div className="font-bold">إرسال ملف مأرشف</div>
                      <div className="text-[10px] text-slate-400">من الأرشيف الإلكتروني للعميل</div>
                    </div>
                  </button>

                  <button
                    type="button"
                    onClick={() => handleOpenShareModal('ACCOUNT_STATEMENT')}
                    className="w-full p-2 rounded-xl hover:bg-blue-950/50 hover:text-blue-300 text-slate-200 text-xs flex items-center gap-2.5 transition-colors cursor-pointer text-right"
                  >
                    <BookOpen className="w-4 h-4 text-blue-400 shrink-0" />
                    <div>
                      <div className="font-bold">إرسال كشف حساب</div>
                      <div className="text-[10px] text-slate-400">تفصيلي بالأرصدة والحركات والمسدد</div>
                    </div>
                  </button>

                  <button
                    type="button"
                    onClick={() => handleOpenShareModal('ANY_FILE')}
                    className="w-full p-2 rounded-xl hover:bg-amber-950/50 hover:text-amber-300 text-slate-200 text-xs flex items-center gap-2.5 transition-colors cursor-pointer text-right"
                  >
                    <Paperclip className="w-4 h-4 text-amber-400 shrink-0" />
                    <div>
                      <div className="font-bold">إرسال أي ملف أحدده</div>
                      <div className="text-[10px] text-slate-400">رفع واختيار أي ملف PDF / Excel / صورة</div>
                    </div>
                  </button>

                  <button
                    type="button"
                    id="btn-attach-send-code"
                    onClick={() => {
                      setIsCodeModalOpen(true);
                      setShowAttachMenu(false);
                    }}
                    className="w-full p-2 rounded-xl hover:bg-teal-950/50 hover:text-teal-300 text-slate-200 text-xs flex items-center gap-2.5 transition-colors cursor-pointer text-right border-t border-slate-800"
                  >
                    <KeyRound className="w-4 h-4 text-teal-400 shrink-0" />
                    <div>
                      <div className="font-bold">إرسال كود من داخل البرنامج</div>
                      <div className="text-[10px] text-slate-400">رمز OTP أو كود توثيق واعتماد فوري</div>
                    </div>
                  </button>
                </div>
              )}
        ...
        {showRightPanel && selectedClient && (
          <div className="lg:col-span-3 border-r border-slate-800 bg-slate-950/90 p-4 space-y-4 overflow-y-auto max-h-[640px] custom-scrollbar">
            ...
            <div className="space-y-3">
              {/* Prominent File & Statement Sharing Section */}
              <div className="bg-slate-900 border border-emerald-500/40 rounded-xl p-3 space-y-2.5 shadow-sm">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-bold text-emerald-300 flex items-center gap-1.5">
                    <Paperclip className="w-3.5 h-3.5 text-emerald-400" />
                    <span>إرسال الملفات وكشوف الحسابات</span>
                  </h4>
                ...
                <div className="grid grid-cols-1 gap-1.5">
                  ...
                  <button
                    type="button"
                    id="btn-quick-send-archive"
                    onClick={() => handleOpenShareModal('ARCHIVED_DOCUMENT')}
                    className="w-full text-right p-2 rounded-lg bg-slate-950 hover:bg-purple-950/40 border border-slate-800 hover:border-purple-500/50 text-[11px] flex items-center justify-between transition-all cursor-pointer group"
                  >
                    <div className="flex items-center gap-2">
                      <FileText className="w-3.5 h-3.5 text-purple-400 shrink-0" />
                      <div>
                        <div className="font-bold text-slate-200 group-hover:text-purple-300">
                          ملف مأرشف من الأرشيف
                        </div>
                        <div className="text-[10px] text-slate-400">
                          {(selectedClient.documents || []).length} مستند مسجل بالأرشيف
                        </div>
                      </div>
                    </div>
                  ...
                  <button
                    type="button"
                    id="btn-quick-send-statement"
                    onClick={() => handleOpenShareModal('ACCOUNT_STATEMENT')}
                    className="w-full text-right p-2 rounded-lg bg-slate-950 hover:bg-blue-950/40 border border-slate-800 hover:border-blue-500/50 text-[11px] flex items-center justify-between transition-all cursor-pointer group"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">
                          الأرصدة والحركات والمسدد والمتبقي
                        </div>
                      </div>
                    </div>
                  ...
                  <button
                    type="button"
                    id="btn-quick-send-anyfile"
                    onClick={() => handleOpenShareModal('ANY_FILE')}
                    className="w-full text-right p-2 rounded-lg bg-slate-950 hover:bg-amber-950/40 border border-slate-800 hover:border-amber-500/50 text-[11px] flex items-center justify-between transition-all cursor-pointer group"
                  >
                    <div className="flex items-center gap-2">
                      <Paperclip className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                      <div>
                        <div className="font-bold text-slate-200 group-hover:text-amber-300">
                          تحديد وإرسال أي ملف من الجهاز
                        </div>
                        <div className="text-[10px] text-slate-400">
                          رفع ملف PDF / Excel / صورة ومشاركته
                        </div>
                      </div>
                    </div>
      ...
      {/* WhatsApp File & Statement Sharing Modal */}
      {isShareModalOpen && (
        <WhatsAppDocumentShareModal
          ...
          state={state}
          initialShareType={shareModalType}
        />
      )}
